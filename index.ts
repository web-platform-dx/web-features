...
function* yamlEntries(root: string): Generator<[string, any]> {
  ...
  for (const fp of filePaths) {
    ...
    if (!identifierPattern.test(key)) {
      throw new Error(
        `${key} is not a valid identifier (must be lowercase a-z, 0-9, and hyphens)`,
      );
    }
...
const groups: { [key: string]: GroupData } = Object.fromEntries(
  yamlEntries("groups"),
);
...
for (const [key, data] of Object.entries(groups)) {
  ...
  while (iter.parent) {
    ...
    if (!iter) {
      throw new Error(
        `group ${chain.at(-2)} refers to parent ${chain.at(-1)} which does not exist.`,
      );
    }
  }
}

const snapshots: { [key: string]: SnapshotData } = Object.fromEntries(
  yamlEntries("snapshots"),
);
...
for (const [key, data] of yamlEntries("features")) {
  ...
  if (data[draft]) {
    if (!data.draft_date) {
      throw new Error(
        `The draft feature ${key} is missing the draft_date field. Set it to the current date.`,
      );
    }
    continue;
  }
  ...
  if (data.name?.length > nameMaxLength) {
    throw new Error(
      `The name field in ${key}.yml is too long, ${data.name.length} characters. The maximum allowed length is ${nameMaxLength}.`,
    );
  }
  if (data.description?.length > descriptionMaxLength) {
    throw new Error(
      `The description field in ${key}.yml is too long, ${data.description.length} characters. The maximum allowed length is ${descriptionMaxLength}.`,
    );
  }
  ...
  for (const group of identifiers(data.group)) {
    if (!Object.hasOwn(groups, group)) {
      throw new Error(
        `group ${group} used in ${key}.yml is not a valid group. Add it to groups/ if needed.`,
      );
    }
  }
  for (const snapshot of identifiers(data.snapshot)) {
    if (!Object.hasOwn(snapshots, snapshot)) {
      throw new Error(
        `snapshot ${snapshot} used in ${key}.yml is not a valid snapshot. Add it to snapshots/ if needed.`,
      );
    }
  }
  ...
  if (data.compat_features) {
    ...
    for (const bcdKey of data.compat_features) {
      ...
      if (otherKey) {
        throw new Error(
          `BCD key ${bcdKey} is used in both ${otherKey} and ${key}, which creates ambiguity for some consumers. Please see https://github.com/web-platform-dx/web-features/issues/1173 and help us find a good solution to allow this.`,
        );
      } else {
...
for (const browser of coreBrowserSet.map((identifier) =>
  compat.browser(identifier),
)) {
