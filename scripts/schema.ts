...
function checkSchemaConsistency(): void {
  ...
  const schemaOnDisk: string = fs.readFileSync(schemaPath, {
    encoding: "utf-8",
  });
  const schemaGenerated: string = child_process
    .execSync("npm run --silent schema", { encoding: "utf-8" })
    .trim();

  if (schemaOnDisk !== schemaGenerated) {
    console.error(
      "There's a mismatch between the schema on disk and types in `index.ts`.",
    );
    console.error(
      "This may produce misleading results for feature validation.",
    );
