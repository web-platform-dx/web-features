...
function updateDistFile(fp: string): void {
  ...
  currentDist.commentBefore = undefined; // Suppress the generated comment for comparisons

  // Write a dist only if it's new or modified
  if (!currentDist || stringify(currentDist) !== stringify(newDist)) {
    newDist.commentBefore = [
      `Generated from: ${id}.yml`,
      `Do not edit this file by hand. Edit the source file instead!`,
      `Last modified: ${Temporal.Now.zonedDateTimeISO("UTC")}`,
    ]
      .map((line) => ` ${line}`)
      .join("\n");

    fs.writeFileSync(distPath, stringify(newDist));
  }
}
